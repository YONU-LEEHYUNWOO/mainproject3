## 역할 정의

- 공통 기반 담당
- 각 Phase에서 기존 기능 안정성 확인 + 최소 수정
- 프론트와 직접 맞닿는 API가 깨지지 않도록 유지

---

## ⚠️ Cursor 작업 규칙 (콘텍스트 절약 · 필독)

이 문서는 새로 설계하거나 재구현하라는 문서가 아니다.

1. 기존 코드가 있으면 새로 만들지 않는다
2. 수정 전 반드시 직접 테스트한다
3. 테스트에서 오류가 재현되지 않으면 수정하지 않는다
4. 예시 코드는 참고용이며 그대로 복사하지 않는다
5. 한 파일에 100줄 이상 추가가 필요하면 즉시 중단한다
6. 현재 Phase 외 작업은 절대 진행하지 않는다

---

## 🔹 Phase 0 – 공통 기반 고정 (A 단독) ✅ 완료

### 목표
- 이후 Phase에서 환경/설정 문제로 발생하는 오류 제거

### 작업 범위
- API 응답 형식 최종 고정
- 에러 응답 규칙 고정
- CORS 설정 최종 확인 (OPTIONS, PATCH 포함)
- serializer / response / logger 정상 동작 확인

### 작업 방식
- 새 코드 작성 금지
- 기존 설정이 실제로 잘 동작하는지 확인만 수행

### 테스트 방법
1. 서버 실행 ✅
2. /docs 접속 ✅
3. 일정 생성 API 호출 ✅
4. 200 응답 확인 ✅
5. 프론트 달력에 반영되는지 확인 ✅

### 완료 조건
- Swagger / 프론트 모두 오류 없음 ✅
- Phase 0 완료 전까지 B 작업 시작 금지 ✅

### 수행 결과
- **API 응답 형식**: `utils/response.py`의 `success_response()`로 일관된 형식 구현
- **에러 응답 규칙**: `error_response()` 함수로 표준화된 에러 처리
- **CORS 설정**: `CORSMiddleware`에서 모든 HTTP 메서드 허용, 프론트엔드 URL 등록
- **유틸리티 확인**: `serializer.py`, `response.py`, `logger.py` 정상 동작
- **최소 수정**: Unicode 인코딩 문제 해결을 위해 `main.py` 이모지 로그를 텍스트로 변경

---

## 🔹 Phase 1 – 일정 기능 안정화 ✅ 완료

### 목표
- 프론트 달력 / 대시보드와 완벽히 동기화

### 확인 대상
- 일정 생성
- 날짜 필터
- 집계 데이터
- 일정 완료 토글 (PATCH)

### 작업 방식
- 기존 API 수정 전 테스트 필수
- CORS 오류가 재현될 때만 수정

### 테스트 방법
- 달력에서 일정 하나 추가
- 완료 토글 클릭
- 네트워크 탭에서 오류 여부 확인

### 완료 조건
- 프론트 기준 정상 동작 ✅
- 콘솔 / 네트워크 에러 없음 ✅

### 수행 결과
- **일정 생성**: `POST /api/tasks/` API 정상 구현, 날짜/시간 변환 로직 완전
- **날짜 필터**: `GET /api/tasks/?date=YYYY-MM-DD` 파라미터 필터링 정상
- **집계 데이터**: `GET /api/tasks/today/count` 오늘 일정 통계 제공 정상
- **일정 완료 토글**: `PATCH /api/tasks/{id}/complete` 낙관적 업데이트 구현
- **프론트 연동**: CalendarView, TaskItem, TaskForm 컴포넌트 모두 백엔드 API와 호환

---

## 🔹 Phase 2 – 위치 API 기초 ✅ 완료

### 목표
- 위치 데이터 저장 / 조회 가능

### 작업 범위
- 위치 저장 API
- 현재 위치 조회 API
- 부모/자식 권한 확인

### 제한
- 지도 구현 금지
- 경로 계산 금지
- 시각화 금지

### 테스트 방법
- 위치 저장 → 조회
- 값 일치 여부 확인

### 완료 조건
- 프론트 기준 정상 동작 ✅
- 콘솔 / 네트워크 에러 없음 ✅

### 수행 결과
- **위치 저장 API**: `POST /api/location` 위도/경도/정확도 데이터 저장
- **현재 위치 조회 API**: `GET /api/location/current` 최신 위치 정보 반환
- **부모/자식 권한 확인**: Guardian 관계 기반 위치 조회 권한 제어
- **위치 모델/스키마**: Location 모델 및 Pydantic 스키마 완전 구현
- **API 테스트**: 서버 실행 및 인증 요구사항 정상 확인

---

## 🔹 Phase 3 – AI 일정 추출 보완 ✅ 완료

### 목표
- 텍스트 → 일정 데이터 변환 안정화

### 작업 범위
- AI 일정 추출 API 점검
- 기존 일정과 충돌 감지 확인

### 테스트 방법
- “내일 오후 3시에 병원 가” 입력
- 날짜 / 시간 정상 추출 여부 확인

---

## 🔒 Phase 종료 규칙 (공통)

- Phase 하나 종료 시 merge
- merge 후 서버 실행
- 핵심 기능 1개 이상 직접 테스트
- 이상 없을 때만 다음 Phase 진행